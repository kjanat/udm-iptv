...
| ----------------------- | ---------------------------------------- | ---------------------------------------------------------------------------- |
| `wan.interface`         | physical uplink                          | yes, passively                                                               |
| `wan.vlan`              | 802.1Q path                              | trial, no RFC                                                                |
...
| `wan.dhcpOptions`       | udhcpc (`-O staticroutes`, `-V IPTV_RG`) | trial                                                                        |
...
Runs when the IPTV interface or WAN already exists, or after a previous installation.

- Links, carrier, skip DSA/bridges, trace the default route (`ppp0` → `eth8.6` → `eth8`).
...
- `/proc/net/ip_mr_cache` (source, group, iif).
- iptables NAT counters on `-o iptv` (first packet per conntrack entry, not traffic volume).
- Initial 30s multicast capture: IGMP queries/reports and any SAP announcements already delivered to the interface. Record the window; absence is inconclusive. SAP destinations depend on scope ([RFC 2974]); treat `224.0.250.64:9875` only as a provider-specific candidate requiring evidence.
...
Already done. Trace the default route to Ethernet, skip DSA/bridges. Dual-WAN: use the board table only as an ordering hint.
...
- Trial (layer B).
- Existing tagged interfaces on WAN with traffic, excluding internet.
...
`br*` with IPv4. Also: which bridge receives IGMP joins during capture. Unchecked bridges do not get TV.
...
1. Passive checks in `diagnose`: available lease evidence and installed routes, an initial 30s SAP/IGMP capture, querier, NAT counters. Store provenance, confidence, observation windows, and unknowns. Redact host identifiers in exported reports separately from exact local operational evidence; redacted prefixes are not discovered network ranges.
2. Fingerprint function: observations → catalog scores or a `custom` candidate, with assumptions and unresolved fields preserved.
3. Wizard: present evidence and assumptions for confirmation, with overrides and required-field resolution before apply.
4. Active VLAN/DHCP trial, initial setup only, after fixing option-121 default-route handling and validating trial isolation, cancellation, cleanup, and preservation of existing network state.
5. Magenta/`ppp0` and existing tagged WAN as special cases.
