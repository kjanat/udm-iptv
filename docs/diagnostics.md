...
Arbitrary log/marker messages and DHCP option payloads are omitted because they can contain credentials or identifying hostnames. Their timestamps and event types remain. Other opaque strings use aliases. The private input remains the source for full local investigation. JSON, text and the viewer use the same sanitized event data. Failure telemetry receives this sanitized attachment while local failure output retains detail. Error telemetry retains exception messages, types and relationships.
