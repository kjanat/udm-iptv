...
- Availability and queryable build information for both proxy programs, including the unselected alternative. Configuration reports and hourly observations carry the structured inventory. Event tags and log/metric attributes include `proxy.improxy.available`, `proxy.igmpproxy.available`, their `.source`, known `.version`, `.package_version`, `.build_id`, `.revision` and feature statuses. Group these by the existing model and firmware tags; a source of `unknown` is a discovery failure, not confirmed absence. Package versions, binary versions and VCS revisions stay separate. Features without build-specific evidence remain unknown.
