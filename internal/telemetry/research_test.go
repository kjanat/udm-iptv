...
func TestResearchOptOutAndForgedEvents(t *testing.T) {
	...
	if err := r.RecordConfiguration(context.Background(), value, true, func(context.Context) NetworkIdentity { t.Fatal("lookup after opt-out"); return NetworkIdentity{} }); err != nil {
		t.Fatal(err)
	}
...
func TestResearchResetAndFeedback(t *testing.T) {
	...
	if err := r.Feedback("arbitrary private text", ""); err == nil {
		t.Fatal("free text accepted")
	}
	if err := r.Feedback("working", "private-customer"); err == nil {
