		...
		for _, interactive := range []bool{false, true} {
			...
			t.Run(name, func(t *testing.T) {
				...
				command := application.installCommandWith(installDependencies{
					...
					prompt:      func(_ context.Context, v *config.Config) error { prompts++; v.WAN.VLAN = 20; return nil },
					executable:  func() (string, error) { return "/tmp/preview-binary", nil },
					requireRoot: func() error { t.Fatal("preview checked root"); return nil }, backend: backend,
				})
