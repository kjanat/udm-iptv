...
func renderVLANCheck(check *vlanCheck) string {
	if check == nil {
		return ""
	}
	...
	switch check.Status {
	case "match":
		return fmt.Sprintf("VLAN configuration matches: %s on %s, VLAN %d (DHCP and playback not verified).\n\n", check.Name, check.Parent, check.ID)
	case "mismatch":
		return "VLAN configuration mismatch:\n" +
