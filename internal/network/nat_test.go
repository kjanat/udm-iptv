...
func TestEnsureNATRemovesLegacyAndStaleRules(t *testing.T) {
	...
	wantRemoved := []NATRule{
		...
		{Destination: "217.166.0.0/16"},
		{Destination: "195.121.0.0/16"},
		{Destination: "195.121.0.0/16", Managed: true},
	}
	if !slices.Equal(removed, wantRemoved) {
		t.Fatalf("removed = %v, want %v", removed, wantRemoved)
	}
	if got := removed[0].String(); got != "unmanaged 213.75.0.0/16: 187 packets, 27452 bytes" {
...
func TestRemoveNATClearsEveryRuleOnTheInterface(t *testing.T) {
	...
	if len(table.rules) != 3 || len(table.deleted) != 6 || len(removed) != 6 {
		t.Fatalf("chain after removal: %q, removed %v", table.rules, removed)
	}
	if removed[0] != (NATRule{Destination: "213.75.0.0/16", Packets: 187, Bytes: 27452}) || removed[4] != (NATRule{Destination: "217.166.0.0/16", Managed: true, Packets: 12, Bytes: 3456}) {
